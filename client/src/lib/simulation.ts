// ======= Constants & Utils =======
const ATTRS = ["humor", "insight", "bait", "controversy", "news", "dunk"] as const;
const USER_TYPES = ["Normal", "Joker", "Troll", "Intellectual", "Journalist"] as const;
...
export class SimulationEngine {
  ...
  private applyBoosts(eff: Record<Attribute, number>, boosts: Record<Attribute, number>): Record<Attribute, number> {
    const out = { ...eff };
  ...
  private vibeAdjust(eff: Record<Attribute, number>, vibe: boolean, cfg: SimulationConfig): Record<Attribute, number> {
    if (!vibe) return eff;
    const out = { ...eff };
    out.humor = clamp01(out.humor * (1 - cfg.vibeHumorPenalty));
    out.controversy = clamp01(out.controversy * (1 - cfg.vibeControversyPenalty));
  ...
  private reachFromAttrs(
    ...
    const positive = 0.45 * eff.humor + 0.55 * eff.insight + 0.40 * eff.news + 0.25 * eff.dunk;
  ...
  private reactionProbs(eff: Record<Attribute, number>) {
    ...
    const neg = 0.35 * eff.controversy + 0.30 * eff.bait + 0.15 * eff.dunk;
    const amb = 0.30 * eff.controversy + 0.15 * eff.news;
    
    let p_sa = clamp01(0.03 + 0.55 * pos);
    let p_a = clamp01(0.06 + 0.50 * (0.4 * eff.humor + 0.5 * eff.insight + 0.3 * eff.news));
    let p_ns = clamp01(0.02 + 0.35 * amb - 0.05 * eff.insight);
    let p_d = clamp01(0.02 + 0.45 * eff.controversy + 0.05 * eff.bait - 0.05 * eff.insight);
    let p_sd = clamp01(0.01 + 0.30 * neg);
  ...
  private applyHomophily(probs: Record<string, number>, h: number) {
    ...
    let t = out.strong_agree + out.agree + out.not_sure + out.disagree + out.strong_disagree;
    if (t > 1) {
      out.strong_agree /= t;
      out.agree /= t;
      out.not_sure /= t;
  ...
  private commentProb(eff: Record<Attribute, number>, vibeOn: boolean, boost: number): number {
    const hi = 0.40 * eff.controversy + 0.35 * eff.bait + 0.30 * eff.dunk;
  ...
  private sampleReaction(probs: Record<string, number>): string {
    const labels = ["strong_agree", "agree", "not_sure", "disagree", "strong_disagree"];
  ...
  private engagementScore(counts: Record<string, number>, comments: number, user: User, W: SimulationConfig['w']): number {
    const U = TYPE_REACT_UTILITY[user.type];
    const wrBase = W.strong_agree * counts.strong_agree + W.agree * counts.agree + W.not_sure * counts.not_sure + W.disagree * counts.disagree + W.strong_disagree * counts.strong_disagree;
    const wrType = U.SA * counts.strong_agree + U.A * counts.agree + U.NS * counts.not_sure + U.D * counts.disagree + U.SD * counts.strong_disagree;
  ...
  private followerUpdate(
    ...
    const gain = cleared ? cfg.followGainRate * (2 * counts.strong_agree + 1 * counts.agree + 0.2 * comments) : 0;
    const loss = cfg.followLossRate * Math.max(0, baitRatio - cfg.baitRatioThresh / 2) * 10;
  ...
  private runRounds(n: number, cfg: SimulationConfig): void {
    for (let r = 0; r < n; r++) {
      ...
      let rewardSum = 0, baitSum = 0, count = 0;
      const attrSum: Record<Attribute, number> & { n: number } = {
        humor: 0, insight: 0, bait: 0, controversy: 0, news: 0, dunk: 0, n: 0
      };
      const attrSumByType: Record<UserType, Record<Attribute, number> & { n: number }> = {} as any;
      
      for (const t of USER_TYPES) {
        attrSumByType[t] = { humor: 0, insight: 0, bait: 0, controversy: 0, news: 0, dunk: 0, n: 0 };
      }
      ...
      for (const u of this.state.users) {
        ...
        let baitFlags = 0, comments = 0, interactions = 0, penalty = false;
        ...
        for (let i = 0; i < reachBudget; i++) {
          ...
          let probs = isLocal ? this.applyHomophily(probsBase, cfg.homophilyStrength) : { ...probsBase };
          ...
          let tot = probs.strong_agree + probs.agree + probs.not_sure + probs.disagree + probs.strong_disagree;
          if (tot > 1) {
          ...
          if (penalty) {
            for (const k of ['strong_agree', 'agree', 'not_sure', 'disagree', 'strong_disagree']) {
      ...
      this.state.seriesAttrs.push(ATTRS.map(k => (attrSum[k] / Math.max(1, attrSum.n))));

      let totalFollowers = 0;
      for (const t of USER_TYPES) {
        const list = this.state.users.filter(u => u.type === t);
        const avgF = list.reduce((a, u) => a + u.followers, 0) / Math.max(1, list.length);
      ...
      this.state.seriesVibeOverall.push(vibeOnCount / Math.max(1, this.state.users.length));
      for (const t of USER_TYPES) {
